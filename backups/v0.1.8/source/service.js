...
class DocumentsService {
  ...
  async generateDocument(hospitalId, data, userContext) {
    ...
    if (documentType === 'ESTIMATE') {
      ...
      // For INVOICE, RECEIPT, CONSENT_FORM - retrieve estimate as base details for mock compatibility
      const estimate = await this.repository.getEstimateGraph(targetId, hospitalId);
      if (!estimate) {
      ...
      if (documentType === 'INVOICE') {
        // Map Estimate line items onto Invoice schema structure
        const invoiceItems = [];
        estimate.estimateSurgeries.forEach((s) => {
          invoiceItems.push({
            description: s.surgery.surgeryName,
            quantity: 1,
        ...
        estimate.estimateItems.forEach((item) => {
          invoiceItems.push({
            ...
            rate: Number(item.amount) / item.quantity,
            amount: Number(item.amount)
          });
        });

        templateData = {
          invoiceNumber: `INV-${year}-0101`,
          hospital,
          patient: estimate.event.patient,
          createdAt: new Date(),
          status: 'UNPAID',
        ...
        templateData = {
          receiptNumber: `REC-${year}-0101`,
          invoiceNumber: `INV-${year}-0101`,
          hospital,
          patient: estimate.event.patient,
          createdAt: new Date(),
          paymentMode: 'ONLINE / CREDIT CARD',
          transactionRef: 'TXN-998877665544',
          remarks: 'Surgical Treatment & Operating Room Accommodation Fees',
          amountPaid: Number(estimate.grandTotal)
        };
    ...
    // 6. Archival metadata readiness update
    if (documentType === 'ESTIMATE') {
      const generatedFileName = `ESTIMATE-${targetId}-${Date.now()}.pdf`;
      await this.repository.updateEstimateArchival(targetId, {
        generatedFileName,
