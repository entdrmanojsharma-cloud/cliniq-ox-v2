...
const mockPrisma = {
  ...
  invoice: {
    findFirst: async () => ({
      ...
      gstAmount: 6444.00,
      grandTotal: 49844.00,
      patient: { name: 'John Doe', uhid: 'UHID-999' },
      invoiceItems: [
        { description: 'Propofol', quantity: 2, rate: 600.00, amount: 1000.00 }
      ]
    })
  },
  receipt: {
...
const mockRepository = {
  ...
  createDocumentGeneration: async (data) => {
    updatedArchivalData = data;
    return { id: 'doc-gen-123', ...data };
  }
};
...
async function runTests() {
  ...
  try {
    ...
    assert.strictEqual(updatedArchivalData.targetId, 'est-123');
    assert.ok(updatedArchivalData.generatedFileName.startsWith('ESTIMATE-est-123-'));
    assert.strictEqual(updatedArchivalData.generatedBy, 'user-789');
